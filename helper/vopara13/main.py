...
def main():
    ...
    raw_url = "https://ttc.ninja-web.net/vo-para/vo-para13_list.htm"
    ...
    for row in table_rows:
        ...
        if c0text.startswith("VOCALOID PARADISE 13"):
            started = True
            continue
        elif not started:
            continue
        elif c0text.startswith("【"):  # New block
            m = re_block_name.search(c0text)
            if not m:
                print(f"WARNING: invalid row (fake new block)")
                continue
            current_block = sanitize_string(m.group(1))
        elif c0text in ["出展企業", "AHSストア", "v4 flower ＆ 心華", "サークル名"]:  # Skip this row (company name, not a circle)
            # print("Skipped company row")
            continue
        elif c0text.startswith("AHS PARADISE"):
            break # Stop, end of vopara
        else:  #
            if len(col_tags) < 5 or len(col_tags) > 6:
                print(
                    f"WARNING: invalid row (wrong column count: {len(col_tags)})"
                )
                continue
            ...
            sp_number = sanitize_string(col_tags[3].get_text(strip=True))
            booth_number = sanitize_string(col_tags[4].get_text(strip=True))

            comment_parts: list[str] = []
            if main_character:
                comment_parts.append(f"Main character/vocaloid: {main_character}")

            position = f"{current_block}, SP数{sp_number}, {booth_number}"
            circle = Circle(
                aliases=[circle_name],
